...
Your mobile app needs testing. Bugs will draw away your users. They will forgive some bugs. They will demand fixes. But they will uninstall if the bugs make a mobile app unusable.

In this article I will show you the three important building blocks of automated testing for React Native mobile apps:
...
Integrating tests into your mobile app as an after-thought is hard. But it is not impossible. So when you write the first line of code of your new and fresh mobile app, always think about testing. As your code base is growing line by line, so does your set of tests. I am not advocating a 100% test coverage: this means that every line of code is covered by an automated test. The important core components need to be tested. You have to be confident that the main business logic is working.
...
Every test journey start with a single test step.

Start with unit tests first. But what is a unit? A unit is a separate block of code that can be isolated from the rest. In React Native and JavaScript, that could be a class or a file with utility functions. I like to keep it simple and have a unit test file for the source code files that I want to test.
...
Within the first line, the function call `generateRandomLetters(3)` will generate an array of three random letters. The second line will test if the length of the generated array is four. So why not test if the length is three, as it is called within the first function? Because you should intentionally make your first test fail. This will verify that your unit test is running at all. Then you refactor to make the test pass. Since the method is already existing in the first place here, we adopt the unit test. Read this short summary of [test-driven development](https://martinfowler.com/bliki/TestDrivenDevelopment.html) by Martin Fowler.

Run the unit jests with the this command:
...
When your first unit test is passing, you can give yourself a pat on the back. This is your first step in the right direction: writing higher quality source code. From here on you can expand your unit tests, include more files, test more functions. Remember to test one functionality at a time to keep the unit tests concise.

When you're ready, it's time to test whole components.

## Think bigger: Component Testing

What's the goal of component testing? We want to ensure components render and behave as expected. We'll use the [React Native Testing Library (RNTL)](https://github.com/callstack/react-native-testing-library) to achieve this. This library makes it easy to test a whole component without having to write test code for the whole rendering logic by hand. The RNTL integrates seamlessly with Jest, the unit test framework we use before. Start by installing the RNTL:
...
I am not 100% sure if this was related to actual code issues or my test setup being not created with the mobile app initialization. But it is worth overcoming the initial issues. It is easier if you're starting your fresh mobile app already with the component tests so you can adapt teh configuration as you go.

My mobile app _ABZ Game_ has a component called `GameTile`. This component displays a letter inside a rectangle with rounded corners and a linear gradient background. These tiles can be moved on the game grid and result in the next higher letter when combined. I needed a component test for this central element of the game. Similar to unit tests, I created a `__test__` folder next to the component's file and named it `GameTile.test.js`. The content follows the Jest unit test structure.
...
If you encounter a bug that wasn't caught by tests, write a test that would've caught it. Commit and push the fixed code together with the test code. Then take a sip from your coffee knowing that this bug will not occur again.
